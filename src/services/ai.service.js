...
${question}
  ...
  if (result === "OUT_OF_SCOPE") {
    if (/interview|education|skill|experience|job|candidate/i.test(question)) {
...
function getColumnsForSections(sectionsNeeded) {
  if (!Array.isArray(sectionsNeeded) || sectionsNeeded.length === 0)
...
function flattenCandidate(candidate, mode = "summary", sectionsNeeded = null) {
  ...
  const wantedSections = Array.isArray(sectionsNeeded)
    ? new Set(sectionsNeeded)
    : null;
  ...
  if (Array.isArray(candidate.Skills) && candidate.Skills.length > 0) {
    ...
    flat["_allSkills"] = all; // always collect for summary stats
    if (wants("Skills")) {
      const all = candidate.Skills.map((s) => s.Skill).filter(Boolean);
      flat["Skills"] = isDetail
        ? all.join(", ")
        : all.slice(0, 5).join(", ") +
          (all.length > 5 ? ` +${all.length - 5} more` : "");
      flat["_allSkills"] = all; // always set regardless of mode — needed by buildSummary
    }
  }
  // ── Education ─────────────────────────────────────────────────────────────
  if (
  ...
  if (
    ...
    const lines = candidate.Experience.filter(
      (e) => e.companyName && e.companyName.trim() !== "",
    ).map(
      (e) =>
        `${e.companyName} — ${e.role || "N/A"}${e.isCurrentCompany ? " (Current)" : ""}`,
    );
  ...
  if (
    ...
    flat["Shortlisted"] = candidate.Applications.some((a) => a.isShortlisted)
      ? "Yes"
      : "No";
    flat["Accepted"] = candidate.Applications.some((a) => a.isAccepted)
      ? "Yes"
      : "No";

    // Null-guard: only flatMap Interviews when it is actually an array
    const allInterviews = candidate.Applications.flatMap((a) =>
      (Array.isArray(a.Interviews) ? a.Interviews : []).map((iv) => ({
        ...iv,
        JobTitle: a.JobTitle,
      })),
    );
    ...
    if (allInterviews.length > 0) {
      if (isDetail) {
        ...
        const uniqueStatuses = [
          ...new Set(
            allInterviews.map((iv) => iv.InterviewStatus).filter(Boolean),
          ),
        ];
        if (uniqueStatuses.length)
          flat["Interview Status"] = uniqueStatuses.join(", ");
      }
      ...
      if (allMarks.length > 0) {
        ...
        const iv = allInterviews.find(
          (i) => parseFloat(i.marksObtained) === best,
        );
...
function locationStr(r) {
  return (
    [r["City"], r["State"]].filter((v) => v && v !== "—").join(", ") || null
  );
}
...
function buildList(flatRows, totalFound) {
  ...
  const parts = shown.map((r) => {
    ...
    if (r["Skills"] && r["Skills"] !== "—")
      segments.push(`and skills in ${r["Skills"]}`);
  ...
  if (remaining > 0)
    parts.push(
      `and **${remaining} more candidate${remaining > 1 ? "s" : ""}**`,
    );
  return parts.join("; ") + ".";
}
...
function buildSkillsOnly(flatRows, totalFound, sectionsNeeded) {
  ...
  const featuredField = sec.includes("Skills")
    ? "Skills"
    : sec.includes("Education")
      ? "Education"
      : sec.includes("Experience")
        ? "Experience"
        : sec.includes("Applications.Interviews")
          ? "Interview Details"
          : sec.includes("Applications")
            ? "Jobs Applied"
            : sec.includes("Documents")
              ? "Documents Uploaded"
              : sec.includes("Resumes")
                ? "Resumes Uploaded"
                : "Skills";
...
function buildFullDetails(flatRows) {
  ...
  const profiles = flatRows.map((r) => {
    ...
    const loc = locationStr(r);
    const nameLabel =
      nameCounts[r["Name"]] > 1 && r["Email"] && r["Email"] !== "—"
        ? `**${r["Name"]}** (${r["Email"]})`
        : `**${r["Name"]}**`;
    sentences.push(`${nameLabel}${loc ? ` lives in ${loc}` : ""}.`);
    if (r["Email"] && r["Email"] !== "—")
      sentences.push(`Email: ${r["Email"]}, Phone: ${r["Phone"] || "—"}.`);
    ...
    if (r["Interview Details"] && r["Interview Details"] !== "—")
      sentences.push(
        `Interview: ${r["Interview Details"].replace(/\n/g, " | ")}.`,
      );
...
function buildCompare(flatRows, totalFound) {
  ...
  const eduDiff = compared.some(
    (r) => r["Education"] !== compared[0]["Education"],
  );
  if (eduDiff) {
    const eduParts = compared.map(
      (r) =>
        `**${r["Name"]}** has ${degreeShort(r["Education"]) || "no degree listed"}`,
    );
  ...
  const skillParts = compared.map((r) => {
    const s =
      r["Skills"] && r["Skills"] !== "—" ? r["Skills"] : "no skills listed";
  ...
  const locDiff = compared.some(
    (r) => locationStr(r) !== locationStr(compared[0]),
  );
  if (locDiff) {
  ...
  if (remaining > 0)
    lines.push(
      `*${remaining} more candidate${remaining > 1 ? "s" : ""} not shown. Generate a report for the full list.*`,
    );
...
function buildSummary(flatRows, totalFound) {
  ...
  const hasUG = flatRows.filter(
    (r) => r["Education"] && r["Education"].includes("UG:"),
  ).length;
  const hasPG = flatRows.filter(
    (r) => r["Education"] && r["Education"].includes("PG:"),
  ).length;
  const hasExp = flatRows.filter(
  ...
  const cities = [
    ...new Set(flatRows.map((r) => r["City"]).filter((c) => c && c !== "—")),
  ];
  ...
  if (hasExp > 0 || noExp > 0) {
    lines.push(
      `• **${hasExp}** have work experience, **${noExp}** are freshers.`,
    );
  }
  if (shortlisted > 0)
    lines.push(
      `• **${shortlisted}** are shortlisted${accepted > 0 ? `, **${accepted}** have been accepted` : ""}.`,
    );
  if (cities.length > 0) {
    const cityStr =
      cities.length <= 4
...
function formatTable(flatRows, options = {}) {
  ...
  if (
    ...
    priorityCols = isSingleCandidate
      ? DETAIL_COLUMNS
      : [
          "Name",
          "City",
...
export async function generateNaturalAnswer(
  ...
  const responseType =
    ...
    detectResponseType(question, isSingleCandidate, totalFound, sectionsNeeded);
...
`,
    ...
    const finalAnswer = fullList ? `${fullList}\n\n---\n\n${aiText}` : aiText;
    ...
    .map((c) => {
      ...
      if (sec.has("Profile")) {
        ...
        if (p.dateOfBirth) {
          ...
          lines.push(
            `- **Date of Birth:** ${born} (Born: ${isNaN(dob) ? "?" : dob.getFullYear()})`,
          );
        }
        ...
        if (p.isStudying != null)
          lines.push(
            `- **Currently Studying:** ${p.isStudying ? "Yes" : "No"}`,
          );
        if (p.linkedinProfileUrl)
          lines.push(`- **LinkedIn:** ${p.linkedinProfileUrl}`);
        if (p.portfolioGithubWebsiteUrl)
          lines.push(`- **Portfolio:** ${p.portfolioGithubWebsiteUrl}`);
      }
      ...
      if (sec.has("Applications") && !sec.has("Applications.Interviews")) {
        const apps = (c.Applications || [])
          .map(
            (a) =>
              `${a.JobTitle || "?"}${a.Status ? ` (${a.Status})` : ""}${a.isShortlisted ? " ✓ Shortlisted" : ""}`,
          )
          .join("; ");
      ...
      if (sec.has("Applications.Interviews")) {
        const interviews = (c.Applications || []).flatMap((a) =>
          (Array.isArray(a.Interviews) ? a.Interviews : []).map((iv) => {
            ...
            if (iv.marksObtained != null)
              parts.push(
                `Marks: ${iv.marksObtained}${iv.totalMarks ? `/${iv.totalMarks}` : ""}`,
              );
            if (iv.feedback?.trim())
              parts.push(`Feedback: "${iv.feedback.trim()}"`);
            return parts.join(" | ");
      ...
      if (sec.has("Documents")) {
        ...
        lines.push(
          `- **Documents:** ${uploaded.length ? uploaded.join(", ") : "None uploaded"}`,
        );
      }
      ...
      if (sec.has("Resumes")) {
        ...
        lines.push(
          `- **Resumes:** ${resumes.length ? `${resumes.length} uploaded` : "None uploaded"}`,
        );
      }
